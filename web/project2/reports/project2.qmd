...
```{r}
#| fig-cap: "Distribution of D scores"
...
d_plot <- ggplot(participant_wide, aes(d_score)) +
  ...
  labs(title = "Distribution of D scores", x = "D score", y = "Count") +
  theme_minimal()
print(d_plot)
```
...
plot_df <- data.frame(category = c("congruent", "incongruent"),
                      ...
                      row.names = NULL)
mean_score_barplot <- ggplot(plot_df, aes(category, mean_values)) +
  geom_col(fill = "grey") +
  geom_errorbar(aes(ymin = mean_values - 2*se_values,
                ymax = mean_values + 2*se_values), width = 0.2, col="steelblue") +
  labs(title = "Mean RT on congruent and incongruent trials", x = "Category",
       y = "Mean values") +
  theme_minimal()

print(mean_score_barplot)

```

```{r}
#| fig-cap: "Correlation between anxiety and d_scores"
...
ggsave("mean_score_barplot.png", mean_score_barplot, 
       path = here::here("outputs","plots"))
ggsave("d_plot.png", d_plot, path = here::here("outputs","plots"))
```
